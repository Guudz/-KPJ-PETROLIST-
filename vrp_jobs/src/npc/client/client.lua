...
AddEventHandler("petrol:intrebare", function()
	vRP.notify({"Incarci petrol in cisterna , il rafinezi \n si il vinzi , simplu!"})
end)

RegisterNetEvent("petrol:intrebare2")
AddEventHandler("petrol:intrebare2", function()
	vRP.notify({"Desigur , uniforma este obligatorie in caz de ceva."})
end)
RegisterNetEvent("petrol:intrebare3")
AddEventHandler("petrol:intrebare3", function()
	vRP.notify({"Tirul si remorca pot fi cumparate de la Showroom \n du-te unde ti-am pus eu."})
	...
	if hasjob then
		exports.rprogress:Custom({
			...
			onComplete = function(cancelled)
				...
				vRP.Notify(player, "Ai intrat pe tura")
				Wait(3000)
				vRP.notify({"Du-te si incarca cisterna!"})
...
AddEventHandler("petrol:iesetura", function()
	exports.rprogress:Custom({
		...
		onComplete = function(cancelled)
			vRP.Notify(player, "Ai iesit din tura")
